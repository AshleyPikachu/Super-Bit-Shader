...
// Core Palette - Standard Palette
   vec3 white  = vec3(1.,0.973,0.886);
   vec3 first = vec3(0.541,0.843,0.678);
   vec3 third = vec3(0.349,0.537,0.855);
   vec3 black  = vec3(0.059,0.008,0.188); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.,0.729,0.761);
   vec3 fourth = vec3(0.275,0.49,0.863);
   vec3 fifth = vec3(0.404,0.4,0.757);
   vec3 ninth = vec3(0.129,0.31,0.451);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.349,0.341,0.635);
   vec3 seventh = vec3(0.325,0.302,0.596);
   vec3 eighth = vec3(0.153,0.314,0.518);
   vec3 tenth = vec3(0.078,0.263,0.392);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.078,0.208,0.345);
   vec3 twelfth = vec3(0.09,0.176,0.345);
   vec3 thirteenth = vec3(0.106,0.133,0.337);
   vec3 fourteenth = vec3(0.075,0.059,0.239);
