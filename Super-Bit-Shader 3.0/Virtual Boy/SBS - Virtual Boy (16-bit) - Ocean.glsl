...
// Core Palette - Standard Palette
   ...
   vec3 third = vec3(0.431,0.525,0.816);
   vec3 black  = vec3(0.,0.024,0.2); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.047,0.714,0.706);
   vec3 fourth = vec3(0.384,0.475,0.82);
   vec3 fifth = vec3(0.204,0.439,0.78);
   vec3 ninth = vec3(0.227,0.271,0.447);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.196,0.373,0.655);
   vec3 seventh = vec3(0.169,0.333,0.62);
   vec3 eighth = vec3(0.192,0.298,0.506);
   vec3 tenth = vec3(0.22,0.231,0.396);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.18,0.188,0.345);
   vec3 twelfth = vec3(0.09,0.153,0.349);
   vec3 thirteenth = vec3(0.,0.122,0.341);
   vec3 fourteenth = vec3(0.,0.071,0.251);
