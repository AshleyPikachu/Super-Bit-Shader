...
// Core Palette - Standard Palette
   vec3 white  = vec3(0.996,0.98,0.89);
   vec3 first = vec3(0.886,0.765,0.451);
   vec3 third = vec3(0.82,0.388,0.443);
   vec3 black  = vec3(0.063,0.,0.184); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.871,0.557,0.337);
   vec3 fourth = vec3(0.812,0.31,0.369);
   vec3 fifth = vec3(0.725,0.263,0.471);
   vec3 ninth = vec3(0.4,0.231,0.396);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.612,0.227,0.4);
   vec3 seventh = vec3(0.573,0.184,0.353);
   vec3 eighth = vec3(0.471,0.216,0.373);
   vec3 tenth = vec3(0.31,0.196,0.361);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.259,0.145,0.314);
   vec3 twelfth = vec3(0.196,0.125,0.322);
   vec3 thirteenth = vec3(0.149,0.086,0.314);
   vec3 fourteenth = vec3(0.086,0.039,0.227);
