...
// Core Palette - Standard Palette
   vec3 white  = vec3(0.996,0.996,0.996);
   vec3 first = vec3(0.78,0.78,0.78);
   vec3 third = vec3(0.529,0.529,0.529);
   vec3 black  = vec3(0.,0.,0.); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.635,0.635,0.635);
   vec3 fourth = vec3(0.486,0.486,0.486);
   vec3 fifth = vec3(0.435,0.435,0.435);
   vec3 ninth = vec3(0.29,0.29,0.29);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.369,0.369,0.369);
   vec3 seventh = vec3(0.333,0.333,0.333);
   vec3 eighth = vec3(0.306,0.306,0.306);
   vec3 tenth = vec3(0.239,0.239,0.239);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.192,0.192,0.192);
   vec3 twelfth = vec3(0.165,0.165,0.165);
   vec3 thirteenth = vec3(0.129,0.129,0.129);
   vec3 fourteenth = vec3(0.078,0.078,0.078);
