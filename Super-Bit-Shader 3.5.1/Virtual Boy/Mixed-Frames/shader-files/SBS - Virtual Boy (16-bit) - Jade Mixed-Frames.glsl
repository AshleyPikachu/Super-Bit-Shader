...
// Core Palette - Standard Palette
   vec3 white  = vec3(1.,0.984,0.741);
   vec3 first = vec3(0.596,0.855,0.212);
   vec3 third = vec3(0.,0.616,0.969);
   vec3 black  = vec3(0.,0.114,0.027); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.,0.765,0.58);
   vec3 fourth = vec3(0.,0.541,0.894);
   vec3 fifth = vec3(0.,0.537,0.667);
   vec3 ninth = vec3(0.,0.353,0.22);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.,0.463,0.58);
   vec3 seventh = vec3(0.,0.424,0.561);
   vec3 eighth = vec3(0.,0.376,0.4);
   vec3 tenth = vec3(0.,0.294,0.129);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.,0.247,0.11);
   vec3 twelfth = vec3(0.,0.212,0.);
   vec3 thirteenth = vec3(0.,0.173,0.);
   vec3 fourteenth = vec3(0.,0.133,0.);
