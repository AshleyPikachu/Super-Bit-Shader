// Super Bit Shader - 5-Bit
// Recommended Console: Pokémon Mini (PokeMini)
...
//-------------------------------------------------------------------------------------------------
// POKEMON MINI AND COLOR
//-------------------------------------------------------------------------------------------------
// The Pokemon Mini can display 5 shades of monochrome colors, it uses ghosting to create its palette.
...
#pragma parameter white_thresh "White Threshold (everything brighter is white)" 0.50 0.0 1.0 0.01
#pragma parameter first_thresh "1st Threshold" 0.24 0.0 1.0 0.01
#pragma parameter second_thresh "2nd Threshold" 0.10 0.0 1.0 0.01
#pragma parameter third_thresh "3rd Threshold" 0.05 0.0 1.0 0.01
...
{
   // palette
   vec3 white  = vec3(0.612,0.698,0.612); // White
   vec3 first = vec3(0.353,0.443,0.353);
   vec3 second = vec3(0.192,0.286,0.192);
   vec3 third = vec3(0.094,0.188,0.094);
   vec3 black  = vec3(0.063,0.141,0.063); // Black
...
// black does not have its own threshold listed here and is just added at the end.
   ...
   (luma < second_thresh && luma >= third_thresh) ? third : black;
   FragColor.a = 1.0;
}
#endif