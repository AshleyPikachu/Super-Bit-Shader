...
{
// Core Palette
   vec3 white  = vec3(1.,0.996,0.804);
   vec3 first = vec3(1.,0.929,0.694);
   vec3 second = vec3(0.576,0.933,0.984);
   vec3 third = vec3(0.396,0.875,1.);
   vec3 fourth = vec3(0.698,0.714,0.984);
   vec3 fifth = vec3(0.588,0.655,1.);
   vec3 sixth = vec3(0.,0.682,0.702);
   vec3 seventh = vec3(0.,0.627,0.541);
   vec3 eighth = vec3(0.,0.482,0.765);
   vec3 ninth = vec3(0.,0.416,0.733);
   vec3 tenth = vec3(0.235,0.306,0.667);
   vec3 eleventh = vec3(0.102,0.247,0.616);
   vec3 twelfth = vec3(0.,0.224,0.271);
   vec3 thirteenth = vec3(0.,0.153,0.059);
   vec3 fourteenth = vec3(0.,0.043,0.298);
   vec3 black  = vec3(0.,0.,0.188);
