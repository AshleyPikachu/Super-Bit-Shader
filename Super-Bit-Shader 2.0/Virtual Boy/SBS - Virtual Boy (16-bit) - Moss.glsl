...
// Core Palette - Standard Palette
   ...
   vec3 third = vec3(0.227,0.608,0.576);
   vec3 black  = vec3(0.,0.063,0.208); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.416,0.718,0.486);
   vec3 fourth = vec3(0.,0.58,0.608);
   vec3 fifth = vec3(0.,0.51,0.675);
   vec3 ninth = vec3(0.,0.329,0.384);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.,0.424,0.612);
   vec3 seventh = vec3(0.,0.38,0.6);
   vec3 eighth = vec3(0.,0.353,0.463);
   vec3 tenth = vec3(0.,0.282,0.314);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.,0.235,0.278);
   vec3 twelfth = vec3(0.,0.216,0.255);
   vec3 thirteenth = vec3(0.,0.192,0.29);
   vec3 fourteenth = vec3(0.,0.114,0.251);
