...
{
// Core Palette
   vec3 white  = vec3(1.,0.957,1.);
   vec3 first = vec3(0.941,0.902,1.);
   vec3 second = vec3(0.973,0.859,0.612);
   vec3 third = vec3(0.996,0.765,0.412);
   vec3 fourth = vec3(1.,0.604,0.475);
   vec3 fifth = vec3(0.965,0.553,0.396);
   vec3 sixth = vec3(0.761,0.569,0.282);
   vec3 seventh = vec3(0.796,0.447,0.345);
   vec3 eighth = vec3(0.478,0.416,0.725);
   vec3 ninth = vec3(0.494,0.31,0.69);
   vec3 tenth = vec3(0.576,0.145,0.416);
   vec3 eleventh = vec3(0.525,0.016,0.275);
   vec3 twelfth = vec3(0.29,0.153,0.008);
   vec3 thirteenth = vec3(0.259,0.047,0.004);
   vec3 fourteenth = vec3(0.071,0.,0.29);
   vec3 black  = vec3(0.059,0.,0.196);
