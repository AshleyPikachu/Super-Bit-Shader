...
{
// Core Palette
   vec3 white  = vec3(1.,0.992,0.918);
   vec3 first = vec3(0.98,0.933,0.808);
   vec3 second = vec3(0.741,0.906,0.929);
   vec3 third = vec3(0.596,0.851,0.941);
   vec3 fourth = vec3(0.718,0.722,0.878);
   vec3 fifth = vec3(0.624,0.659,0.914);
   vec3 sixth = vec3(0.333,0.659,0.667);
   vec3 seventh = vec3(0.22,0.604,0.541);
   vec3 eighth = vec3(0.275,0.475,0.667);
   vec3 ninth = vec3(0.212,0.408,0.635);
   vec3 tenth = vec3(0.286,0.314,0.573);
   vec3 eleventh = vec3(0.196,0.251,0.525);
   vec3 twelfth = vec3(0.067,0.208,0.227);
   vec3 thirteenth = vec3(0.047,0.137,0.094);
   vec3 fourteenth = vec3(0.004,0.047,0.22);
   vec3 black  = vec3(0.016,0.,0.118);
