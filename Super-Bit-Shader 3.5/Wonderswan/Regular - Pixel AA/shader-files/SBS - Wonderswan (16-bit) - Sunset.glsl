...
#pragma parameter third_thresh "3rd Threshold" 0.80 0.0 1.0 0.01
#pragma parameter fourth_thresh "4th Threshold" 0.73 0.0 1.0 0.01
#pragma parameter fifth_thresh "5th Threshold" 0.61 0.0 1.0 0.01
...
{
// Core Palette
   vec3 white  = vec3(0.996,0.973,0.839);
   vec3 first = vec3(0.949,0.925,0.792);
   vec3 second = vec3(0.941,0.855,0.8);
   vec3 third = vec3(0.878,0.792,0.741);
   vec3 fourth = vec3(0.835,0.718,0.416);
   vec3 fifth = vec3(0.769,0.655,0.357);
   vec3 sixth = vec3(0.863,0.506,0.447);
   vec3 seventh = vec3(0.788,0.439,0.384);
   vec3 eighth = vec3(0.592,0.384,0.576);
   vec3 ninth = vec3(0.545,0.31,0.529);
   vec3 tenth = vec3(0.271,0.325,0.486);
   vec3 eleventh = vec3(0.208,0.267,0.424);
   vec3 twelfth = vec3(0.318,0.118,0.204);
   vec3 thirteenth = vec3(0.235,0.051,0.137);
   vec3 fourteenth = vec3(0.071,0.027,0.22);
   vec3 black  = vec3(0.008,0.,0.153);
