...
{
// Core Palette
   vec3 white  = vec3(1.,0.996,0.694);
   vec3 first = vec3(0.965,0.949,0.651);
   vec3 second = vec3(0.698,0.925,0.757);
   vec3 third = vec3(0.635,0.863,0.694);
   vec3 fourth = vec3(0.565,0.796,0.208);
   vec3 fifth = vec3(0.498,0.733,0.133);
   vec3 sixth = vec3(0.,0.722,0.502);
   vec3 seventh = vec3(0.,0.651,0.439);
   vec3 eighth = vec3(0.,0.525,0.694);
   vec3 ninth = vec3(0.,0.455,0.624);
   vec3 tenth = vec3(0.231,0.349,0.373);
   vec3 eleventh = vec3(0.165,0.286,0.31);
   vec3 twelfth = vec3(0.,0.192,0.396);
   vec3 thirteenth = vec3(0.,0.129,0.29);
   vec3 fourteenth = vec3(0.,0.098,0.031);
   vec3 black  = vec3(0.,0.071,0.);
