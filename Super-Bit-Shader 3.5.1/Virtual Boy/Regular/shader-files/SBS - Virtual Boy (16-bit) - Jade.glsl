...
// Core Palette - Standard Palette
   vec3 white  = vec3(0.992,0.98,0.894);
   vec3 first = vec3(0.682,0.827,0.463);
   vec3 third = vec3(0.224,0.588,0.78);
   vec3 black  = vec3(0.004,0.082,0.059); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.22,0.714,0.608);
   vec3 fourth = vec3(0.247,0.525,0.745);
   vec3 fifth = vec3(0.,0.514,0.588);
   vec3 ninth = vec3(0.102,0.329,0.255);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.,0.435,0.498);
   vec3 seventh = vec3(0.,0.4,0.471);
   vec3 eighth = vec3(0.008,0.349,0.357);
   vec3 tenth = vec3(0.094,0.271,0.188);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.059,0.22,0.161);
   vec3 twelfth = vec3(0.125,0.188,0.094);
   vec3 thirteenth = vec3(0.067,0.153,0.086);
   vec3 fourteenth = vec3(0.055,0.114,0.047);
