...
// Core Palette - Standard Palette
   vec3 white  = vec3(1.,0.973,0.733);
   vec3 first = vec3(0.141,0.886,0.588);
   vec3 third = vec3(0.227,0.525,1.);
   vec3 black  = vec3(0.,0.016,0.302); // Darkest Shade Period.

// Dark Palette - Secondary Palette, it's typically mixed with the Core Palette for dimmer games.
   vec3 second = vec3(0.,0.765,0.776);
   vec3 fourth = vec3(0.255,0.471,0.969);
   vec3 fifth = vec3(0.,0.443,0.918);
   vec3 ninth = vec3(0.102,0.267,0.584);

// Tansition Palette - Used to create transitions between the Core and Dark Palettes.
   vec3 sixth = vec3(0.,0.376,0.8);
   vec3 seventh = vec3(0.,0.337,0.765);
   vec3 eighth = vec3(0.,0.302,0.647);
   vec3 tenth = vec3(0.137,0.224,0.537);
   
// Black Tansition Palette - Used for fade to black transitions.
   vec3 eleventh = vec3(0.118,0.18,0.459);
   vec3 twelfth = vec3(0.,0.149,0.537);
   vec3 thirteenth = vec3(0.,0.122,0.478);
   vec3 fourteenth = vec3(0.,0.067,0.361);
