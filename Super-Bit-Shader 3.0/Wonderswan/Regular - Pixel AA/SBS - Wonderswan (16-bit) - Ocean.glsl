...
{
// Core Palette
   vec3 white  = vec3(0.996,0.957,0.827);
   vec3 first = vec3(0.949,0.91,0.78);
   vec3 second = vec3(0.757,0.894,0.984);
   vec3 third = vec3(0.694,0.831,0.922);
   vec3 fourth = vec3(0.412,0.808,0.608);
   vec3 fifth = vec3(0.349,0.745,0.549);
   vec3 sixth = vec3(0.212,0.667,0.718);
   vec3 seventh = vec3(0.027,0.6,0.655);
   vec3 eighth = vec3(0.443,0.431,0.663);
   vec3 ninth = vec3(0.38,0.373,0.624);
   vec3 tenth = vec3(0.267,0.333,0.459);
   vec3 eleventh = vec3(0.224,0.271,0.357);
   vec3 twelfth = vec3(0.004,0.216,0.247);
   vec3 thirteenth = vec3(0.004,0.141,0.169);
   vec3 fourteenth = vec3(0.004,0.047,0.224);
   vec3 black  = vec3(0.,0.,0.157);
