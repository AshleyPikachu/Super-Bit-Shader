...
{
// Core Palette
   vec3 white  = vec3(1.,0.988,0.847);
   vec3 first = vec3(0.953,0.941,0.804);
   vec3 second = vec3(0.773,0.906,0.804);
   vec3 third = vec3(0.71,0.843,0.741);
   vec3 fourth = vec3(0.647,0.773,0.451);
   vec3 fifth = vec3(0.584,0.71,0.392);
   vec3 sixth = vec3(0.184,0.682,0.545);
   vec3 seventh = vec3(0.067,0.616,0.482);
   vec3 eighth = vec3(0.212,0.494,0.576);
   vec3 ninth = vec3(0.173,0.427,0.506);
   vec3 tenth = vec3(0.282,0.341,0.353);
   vec3 eleventh = vec3(0.22,0.278,0.29);
   vec3 twelfth = vec3(0.11,0.192,0.337);
   vec3 thirteenth = vec3(0.067,0.129,0.235);
   vec3 fourteenth = vec3(0.008,0.078,0.055);
   vec3 black  = vec3(0.,0.047,0.016);
