...
{
// Core Palette
   vec3 white  = vec3(1.,0.988,0.675);
   vec3 first = vec3(0.847,0.98,0.698);
   vec3 second = vec3(1.,0.839,0.631);
   vec3 third = vec3(0.992,0.765,0.471);
   vec3 fourth = vec3(0.482,0.808,0.431);
   vec3 fifth = vec3(0.137,0.757,0.522);
   vec3 sixth = vec3(0.482,0.655,0.314);
   vec3 seventh = vec3(0.357,0.608,0.31);
   vec3 eighth = vec3(0.,0.537,0.639);
   vec3 ninth = vec3(0.,0.463,0.6);
   vec3 tenth = vec3(0.506,0.243,0.196);
   vec3 eleventh = vec3(0.478,0.145,0.102);
   vec3 twelfth = vec3(0.145,0.208,0.);
   vec3 thirteenth = vec3(0.039,0.153,0.);
   vec3 fourteenth = vec3(0.235,0.,0.133);
   vec3 black  = vec3(0.18,0.,0.098);
